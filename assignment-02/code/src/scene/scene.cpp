...
Scene::Scene(Window & window)
    : skybox(geometricFactory),
      ...
      cursor(window)
{
    Camera::instance().initialise({3.1f, 2.75f, 4.3f}, {0.0f, 1.0f, 0.0f});
    ...
    skybox.program.use();
    skybox.projectionLoc.setMat4f(perspective);

    orbs.program.use();
    locs.orbsProjection.setMat4f(perspective);

    orbs.particlesProgram.use();
    locs.particlesProjection.setMat4f(perspective);
    ...
    /*
     * Switch off the torch and main light initially, to best show off the orbs
     * as they float upwards.
     */
    objects.graph.getTorch().togglePower();
 ...
 *
 * E: pauses the particle emission.
 *
 * F5: toggle Blinn Phong Specular.
 *
 * F6 and Shift+F6: adjust bloom level (number of Gaussian blur taps).
 *
 * F7 and Shift+F7: cycles between tone mapping techniques.
 *
 * F8: toggles off and on normal mapping.
 *
    ...
    const int action, const int mods)
{
    ...
    switch (key)
    {
        ...
        case GLFW_KEY_F5:
        {
            objects.graph.toggleBlinnPhongSpecular();
            std::cout << std::format("Blinn Phong specular {}\n",
                objects.graph.getUseBlinnPhongSpecular() ? "on" : "off");
        }
        break;
        ...
        case GLFW_KEY_F6:
        {
            ...
            std::cout << std::format(
                "Using Optimised {}-Tap Kernel Gaussian blur\n",
                window.getNumBlurTaps());
        }
        break;
        ...
        case GLFW_KEY_F7:
        {
            ...
            switch (window.getToneMappingTechnique())
            {
                case 1: technique = "Reinhard"; break;
                case 2: technique = "ACES"; break;
                case 3: technique = "Uncharted2"; break;
                default: technique = "none"; break;
            }

            std::cout << std::format( "Tone mapping technique: {}\n",
                technique);
        }
        break;
        ...
        case GLFW_KEY_F8:
        {
            objects.graph.toggleNormalsMapping();
            std::cout << std::format("Normal mapping {}\n",
                objects.graph.getShowNormalsMapping() ? "on" : "off");
        }
        break;
        ...
        case GLFW_KEY_E: orbs.graph.toggleEmission(); break;
...
Scene::onLoopEvent(Window & window)
{
    ...
    skybox.viewLoc.setMat4f(Camera::instance().getViewMatrix());
    skybox.textureLoc.set1i(0);
    skybox.texture.bind(GL_TEXTURE0);
    skybox.mesh.draw();
    ...
    // render the particles last
    if (objects.graph.getShowOrbs())
    {
        orbs.particlesProgram.use();
        orbs.graph.renderParticles(deltaTime, Camera::instance());
    }
}
 ...
 */
void
Scene::onWindowSizeEvent(Window & window, const GLint width, const GLint height)
{
    const auto perspective = window.getProjection().getPerspective();
