...
#include "../../glo/scene/animators/rocking.h"
...
{
    auto room = modelFactory.create("room.obj");
    sceneNode->addChild(room);

    auto cradle = modelFactory.create("cradle.obj");
    cradle->addAnimator(std::make_shared<RockingAnimator>())
        .setScaling({0.3f, 0.3f, 0.3f})
        .setTranslation({1.25f, 0.32318f, 1.4f});
    sceneNode->addChild(cradle);

    auto golfBall = modelFactory.create("golfBall.obj");
    golfBall->setScaling({0.75f, 0.75f, 0.75f})
        .setTranslation({1.4499f, 0.54067f, -1.1162f});
    sceneNode->addChild(golfBall);

    auto mobile = CotMobileFactory(geometricFactory).create();
    mobile->setScaling({0.3f, 0.3f, 0.3f})
        .setScaling({0.1f, 0.1f, 0.1f})
        .setTranslation({1.15f, 1.5f, 1.4f});
    sceneNode->addChild(mobile);

    auto title = geometricFactory.getDecal("title.png");
    title->setScaling({2.0f, 2.0f, 1.0f})
        .setRotation({0.0f, glm::radians(90.0f), 0.0f})
        .setTranslation({-2.515f, 1.498f, -0.2223f});
    sceneNode->addChild(title);
}
...
{
    auto animator = sceneNode->getChildren()[2]->getAnimators()[0];
    ...
    const OrbsGraph & orbsGraph, const MainLightGraph & mainLightGraph)
{
    ...
    for (int lightIdx = 0; lightIdx < numOrbs; lightIdx++)
    {
        ...
        const auto & colour = orb->getRenderables()[0]->getMaterial()
    ...
    for (int lightIdx = 0; lightIdx < mainLights.size(); lightIdx++)
    {
        ...
        const auto & colour = light->getRenderables()[0]->getMaterial()
            .getBaseColour().value().getRgb();
...
ObjectsGraph::render(std::shared_ptr<SceneNode> node)
{
    for ( auto renderable : node->getRenderables() )
    {
        ...
        {
            locs.texture.set1i(0);
            material.getTexture().value()->bind(GL_TEXTURE0);
        }

        if (showNormalsMapping && material.hasNormals())
        {
            locs.hasNormals.set1b(true);
            locs.normals.set1i(1);
            material.getNormals().value()->bind(GL_TEXTURE1);
        }
        else
        {
            locs.hasNormals.set1b(false);
        }
    }
