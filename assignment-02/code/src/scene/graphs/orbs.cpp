...
#include "../../glo/scene/emitters/trail.h"
#include "../../glo/scene/factories/geometric.h"

OrbsGraph::OrbsGraph(const Program & orbsProgram,
    const Program & particlesProgram, GeometricFactory & geometricFactory)
    : locs(orbsProgram, particlesProgram)
{
    // Five different colours, five different orbs
    // clang-format off
    const std::vector<const char *> colours{
        "#F00", "#FF0", "#0F0", "#0FF", "#BF00FF"};
    // clang-format on
    ...
    for (auto i = 0; i < colours.size(); i++)
    {
        ...
        light->addEmitter(std::make_shared<TrailEmitter>());
        ...
        const LightProperties lp{glm::vec3(0.0f), 0.001f, 0.5f,
            {1.00f, 1.00f}, {0.0f, 0.0f}};
        orbProperties.push_back(lp);
    }

    sceneNode->setScaling({0.3f, 0.3f, 0.3f})
...
{
    locs.orbsView.setMat4f(camera.getViewMatrix());
...
{
    locs.orbsModel.setMat4f(node->getWorldMatrix());
    node->render();
...
{
    locs.particlesView.setMat4f(camera.getViewMatrix());
    if (!pauseEmission) sceneNode->emit(deltaTime);
    renderParticles(sceneNode);
    return *this;
}
 ...
 */
void
OrbsGraph::renderParticles(std::shared_ptr<SceneNode> node)
{
    node->renderParticles();
    for (auto child : node->getChildren()) renderParticles(child);
}
