...
template <typename T> class MeshImpl : public Mesh
{
  public:
    ...
    /*
     * Sets tangents for the *current* VAO. Only necessary when normal mapping
     * is required. The vertices must already be set, and the data must not be
     * empty.
     */
    void setTangents(std::vector<glm::vec3> data)
    {
        assert(!assemblies.empty());
        assert(!data.empty());

        tangents.emplace_back(data);
        assemblies.back()->setTangents(tangents.back());
    }

    /*
     * Sets bitangents for the *current* VAO. Only necessary when normal mapping
     * is required. The vertices must already be set, and the data must not be
     * empty.
     */
    void setBitangents(std::vector<glm::vec3> data)
    {
        assert(!assemblies.empty());
        assert(!data.empty());

        bitangents.emplace_back(data);
        assemblies.back()->setBitangents(bitangents.back());
    }
     ...
     *
     * A reference to colourBuffer is stored to prevent it going out of scope.
     */
    virtual void setColours(std::shared_ptr<ColourBuffer> colourBuffer) override
    {
        assert(!assemblies.empty());
        this->colourBuffer = colourBuffer;
        assemblies.back()->setColours(colourBuffer);
    }
  ...
  private:
    ...
    std::shared_ptr<ColourBuffer> colourBuffer;
    std::vector<BufferImpl<glm::vec3>> tangents;
    std::vector<BufferImpl<glm::vec3>> bitangents;
};
